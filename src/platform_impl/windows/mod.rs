...
unsafe fn get_async_key_state(v_k: u16) -> bool {
    let state = GetAsyncKeyState(v_k as i32) as u16;
    (state & 0x8000) != 0
}
...
unsafe extern "system" fn global_hotkey_proc(
    ...
    if msg == WM_HOTKEY {
        ...
        let mods = hotkey_id >> 16;
        let modifiers = Modifiers::from_bits_truncate(mods);

        let mut matches = true;
        if modifiers.contains(Modifiers::SHIFT_LEFT) && !get_async_key_state(VK_LSHIFT) {
             matches = false;
        }
        if modifiers.contains(Modifiers::SHIFT_RIGHT) && !get_async_key_state(VK_RSHIFT) {
             matches = false;
        }
        if modifiers.contains(Modifiers::CONTROL_LEFT) && !get_async_key_state(VK_LCONTROL) {
             matches = false;
        }
        if modifiers.contains(Modifiers::CONTROL_RIGHT) && !get_async_key_state(VK_RCONTROL) {
             matches = false;
        }
        if modifiers.contains(Modifiers::ALT_LEFT) && !get_async_key_state(VK_LMENU) {
             matches = false;
        }
        if modifiers.contains(Modifiers::ALT_RIGHT) && !get_async_key_state(VK_RMENU) {
             matches = false;
        }
        if modifiers.contains(Modifiers::SUPER_LEFT) && !get_async_key_state(VK_LWIN) {
             matches = false;
        }
        if modifiers.contains(Modifiers::SUPER_RIGHT) && !get_async_key_state(VK_RWIN) {
             matches = false;
        }
